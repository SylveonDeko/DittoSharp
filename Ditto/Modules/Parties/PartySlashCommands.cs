...
namespace Ditto.Modules.Parties
{
    ...
    public class PartyModule(PartyService partyService) : DittoSlashModuleBase<PartyService>
    {
        ...
        public async Task PartySetup([Autocomplete(typeof(PartyNameAutocompleteHandler))] string partyName)
        {
            ...
            var (description, party, partyPokeIds, pokemonIndices, pokemonNames) = await partyService.GetPartySetupData(ctx.User.Id, partyName);
            ...
            components.WithButton(customId: $"party:slot:1:{partyName}", emote: new Emoji("<:1:1013539737014907030>"), label: pokemonNames[0], style: ButtonStyle.Secondary, row: 0);
            components.WithButton(customId: $"party:slot:2:{partyName}", emote: new Emoji("<:2:1013539739263041618>"), label: pokemonNames[1], style: ButtonStyle.Secondary, row: 0);
            components.WithButton(customId: $"party:slot:3:{partyName}", emote: new Emoji("<:3:1013539741502812310>"), label: pokemonNames[2], style: ButtonStyle.Secondary, row: 0);

            // Add slot buttons - Row 2
            components.WithButton(customId: $"party:slot:4:{partyName}", emote: new Emoji("<:4:1013539744027783208>"), label: pokemonNames[3], style: ButtonStyle.Secondary, row: 1);
            components.WithButton(customId: $"party:slot:5:{partyName}", emote: new Emoji("<:5:1013539745692909740>"), label: pokemonNames[4], style: ButtonStyle.Secondary, row: 1);
            components.WithButton(customId: $"party:slot:6:{partyName}", emote: new Emoji("<:6:1013539747517444208>"), label: pokemonNames[5], style: ButtonStyle.Secondary, row: 1);

            // Add close button - Row 3
            components.WithButton(customId: "party:close", label: "Close Menu", emote: new Emoji("<:delete:1051241645447848009>"), style: ButtonStyle.Danger, row: 2);
        ...
        public async Task PartyDeregister([Autocomplete(typeof(PartyNameAutocompleteHandler))] string partyName)
        {
            ...
            // Confirm deletion
            var confirmed = await PromptUserConfirmAsync($"Are you sure you want to deregister party `{partyName}`?", ctx.User.Id);
        ...
        public async Task HandleAddPokemonModal(string slotNumberStr, string partyName, AddPokemonModal modal)
        {
            ...
            catch (Exception ex)
            {
                await ctx.Client.GetChannelAsync(1004311971853779005)
                    .ContinueWith(t => (t.Result as IMessageChannel)?.SendMessageAsync($"**__ERROR OCCURRED__**: ```{ex.Message}\n{ex.StackTrace}```"));

                await FollowupAsync("Something went wrong\nPlease try running this command again.\nJoin the official server in `/ditto info` command if this continues for help.", ephemeral: true);
            }
        }
