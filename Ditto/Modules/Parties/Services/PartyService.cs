...
{
    public class PartyService(DittoContext db, DiscordShardedClient client) : INService
    {
        ...
        public string FormatTable(List<List<string>> rows, List<string> headers)
        {
            ...
            for (var i = 0; i < headers.Count; i++)
            {
                sb.Append(new string('-', columnWidths[i] + 2)).Append('|');
            }
            sb.AppendLine();
        ...
        /// </summary>
        public async Task<(string Description, ulong[] Party, ulong[] PartyPokeIds, int[] PokemonIndices, string[] PokemonNames)> GetPartySetupData(ulong userId, string partyName)
        {
            ...
            if (user?.Pokemon != null)
            {
                for (var i = 0; i < 6; i++)
                {
                    ...
                    if (pokemon != null)
                    {
                        ...
                        if (index != -1)
                        {
                            pokemonIndices[i] = index + 1; // Add 1 for user-friendly indexing
                        }
                    }
                }
            }
            ...
            {
                new List<string> { "1", pokemonIndices[0] > 0 ? pokemonIndices[0].ToString() : "None", pokemonNames[0] },
                new List<string> { "2", pokemonIndices[1] > 0 ? pokemonIndices[1].ToString() : "None", pokemonNames[1] },
                new List<string> { "3", pokemonIndices[2] > 0 ? pokemonIndices[2].ToString() : "None", pokemonNames[2] },
                new List<string> { "4", pokemonIndices[3] > 0 ? pokemonIndices[3].ToString() : "None", pokemonNames[3] },
                new List<string> { "5", pokemonIndices[4] > 0 ? pokemonIndices[4].ToString() : "None", pokemonNames[4] },
                new List<string> { "6", pokemonIndices[5] > 0 ? pokemonIndices[5].ToString() : "None", pokemonNames[5] }
            };
        ...
        public async Task<(bool Success, string Message)> AddPokemonToParty(ulong userId, int slot, int pokeIndex = 0)
        {
            ...
            if (pokemon == null)
            {
                return (false, "You do not have that Pokemon!");
            }
            ...
            if (user.Party.Contains(pokemonId))
            {
        ...
        public async Task<(bool Success, string Message)> DeregisterParty(ulong userId, string partyName)
        {
            ...
            if (party == null)
            {
                return (false, $"You do not have a party with the name `{partyName}`.");
            }
        ...
        public async Task<(bool Success, string Message)> LoadParty(ulong userId, string partyName)
        {
            ...
            if (party == null)
            {
                return (false, "You don't have a party registered with that name.");
            }
        ...
        /// </summary>
        public async Task<(bool Success, string Message)> AddPokemonToPartySlot(ulong userId, int slot, int pokeIndex, string partyName)
        {
            try
            {
    ...
    public class ServiceResult
    {
        ...
        public static ServiceResult FromSuccess(string message) => new ServiceResult { Success = true, Message = message };
        public static ServiceResult FromError(string message) => new ServiceResult { Success = false, Message = message };
    }
}