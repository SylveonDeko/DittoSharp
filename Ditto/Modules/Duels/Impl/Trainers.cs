...
namespace Ditto.Modules.Duels.Impl
{
    ...
    public class Trainer
    {
        ...
        public string NextTurn(Battle battle)
        {
            ...
            if (hp > 0 && CurrentPokemon != null)
            {
                msg += CurrentPokemon.Heal(hp, source: "its wish");
            }
            if (AuroraVeil.NextTurn())
            {
                msg += $"{Name}'s aurora veil wore off!\n";
            }
            if (LightScreen.NextTurn())
            {
                msg += $"{Name}'s light screen wore off!\n";
            }
            if (Reflect.NextTurn())
            {
                msg += $"{Name}'s reflect wore off!\n";
            }
            if (Mist.NextTurn())
            {
                msg += $"{Name}'s mist wore off!\n";
            }
            if (Safeguard.NextTurn())
            {
                msg += $"{Name}'s safeguard wore off!\n";
            }
            if (Tailwind.NextTurn())
            {
                msg += $"{Name}'s tailwind died down!\n";
            }
            if (MudSport.NextTurn())
            {
                msg += $"{Name}'s mud sport wore off!\n";
            }
            if (WaterSport.NextTurn())
            {
                msg += $"{Name}'s water sport evaporated!\n";
            }
            Retaliate.NextTurn();
            ...
            if (FutureSight.NextTurn() && CurrentPokemon != null)
            {
                ...
                var futureSightAttacker = ((Tuple<DuelPokemon, Move>)futureSightData).Item1;
                var futureSightMove = ((Tuple<DuelPokemon, Move>)futureSightData).Item2;
        ...
        {
            if (slot < 0 || slot >= Party.Count)
            {
                throw new ArgumentException("out of bounds");
            }
            if (!(Party[slot].Hp > 0))
            {
                throw new ArgumentException("no hp");
            }
        ...
        public List<int> ValidSwaps(DuelPokemon defender, Battle battle, bool checkTrap = true)
        {
            ...
            {
                if (CurrentPokemon.TypeIds.Contains(ElementType.GHOST))
                {
                    checkTrap = false;
                }
                if (CurrentPokemon.HeldItem.Get() == "shed-shell")
                {
                    checkTrap = false;
                }
                ...
                if (checkTrap)
                {
                    if (CurrentPokemon.Trapping)
                    {
                        return new List<int>();
                    }
                    if (CurrentPokemon.Ingrain)
                    {
                        return new List<int>();
                    }
                    if (CurrentPokemon.FairyLock.Active() || defender.FairyLock.Active())
                    {
                        return new List<int>();
                    }
                    if (CurrentPokemon.NoRetreat)
                    {
                        return new List<int>();
                    }
                    if (CurrentPokemon.Bind.Active() && CurrentPokemon.Substitute == 0)
                    {
                        return new List<int>();
                    }
                    ...
                    if (defender.Ability() == Ability.ARENA_TRAP && CurrentPokemon.Grounded(battle))
                    {
                        return new List<int>();
                    }
                }
        ...
        public ValidMovesResult ValidMoves(DuelPokemon defender)
        {
            ...
            if (CurrentPokemon.LockedMove != null)
            {
                return ValidMovesResult.Forced(CurrentPokemon.LockedMove.Move);
            }
            ...
            for (var idx = 0; idx < CurrentPokemon.Moves.Count; idx++)
            {
                ...
                if (move.Effect == 247 && !CurrentPokemon.Moves.Where(m => m.Effect != 247).All(m => m.Used))
                {
                    continue;
                }
                if (CurrentPokemon.Disable.Active() && move == CurrentPokemon.Disable.Item)
                {
                    continue;
                }
                ...
                if (CurrentPokemon.Torment && CurrentPokemon.LastMove == move)
                {
                    continue;
                }
                ...
                if (defender.Imprison && defender.Moves.Any(x => x.Id == move.Id))
                {
                    continue;
                }
                if (CurrentPokemon.HealBlock.Active() && move.IsAffectedByHealBlock())
                {
                    continue;
                }
                if (CurrentPokemon.Silenced.Active() && move.IsSoundBased())
                {
                    continue;
                }
                switch (move.Effect)
                ...
                if (CurrentPokemon.Encore.Active() && move != CurrentPokemon.Encore.Item)
                {
                    continue;
                }
            ...
            if (result.Count == 0)
            {
                return ValidMovesResult.Struggle();
            }
    ...
    public class NPCTrainer(List<DuelPokemon> party) : Trainer("Trainer John", party)
    {
        ...
        {
            var validSwaps = ValidSwaps(defender, battle, checkTrap: false);
            var pokeIdx = validSwaps[new Random().Next(validSwaps.Count)];
            SwitchPoke(pokeIdx, midTurn: midTurn);

            // Also set this as the selected action if it's not a mid-turn swap
            if (!midTurn)
            {
                SelectedAction = new SwitchAction(pokeIdx);
            }
